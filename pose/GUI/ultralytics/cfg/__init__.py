...
SOLUTIONS_HELP_MSG = f"""
    ...
    2. Call heatmaps solution
        yolo solutions heatmap colormap=cv2.COLORMAP_PARAULA model=model1.pt

    3. Call queue management solution
        yolo solutions queue region=[(20, 400), (1080, 400), (1080, 360), (20, 360)] model=model1.pt

    4. Call workouts monitoring solution for push-ups
        yolo solutions workout model=model2.pt kpts=[6, 8, 10]
...
CLI_HELP_MSG = f"""
    ...
    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        yolo train data=coco8.yaml model=model1.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        yolo predict model=model3.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Val a pretrained detection model at batch-size 1 and image size 640:
        yolo val model=model1.pt data=coco8.yaml batch=1 imgsz=640
...
def parse_key_value_pair(pair: str = "key=value"):
    ...
    Examples:
        >>> key, value = parse_key_value_pair("model=model1.pt")
        >>> print(f"Key: {key}, Value: {value}")
        Key: model, Value: model1.pt
...
def entrypoint(debug=""):
    ...
    Examples:
        ...
        >>> entrypoint("train data=coco8.yaml model=model1.pt epochs=10 lr0=0.01")

        Predict a YouTube video using a pretrained segmentation model at image size 320:
        >>> entrypoint("predict model=model3.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320")

        Validate a pretrained detection model at batch-size 1 and image size 640:
        >>> entrypoint("val model=model1.pt data=coco8.yaml batch=1 imgsz=640")
...
if __name__ == "__main__":
    # Example: entrypoint(debug='yolo predict model=model1.pt')
    entrypoint(debug="")
